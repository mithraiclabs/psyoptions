...
pub fn test_sucessful_close_position() {
    ...
    let initial_option_token_mint_data =
        client.get_account_data(&option_mint_keys.pubkey()).unwrap();
...
#[serial]
/// Option Writer closing out should receive underlying asset
pub fn test_sucessful_close_multiple_positions() {
    // Create the options market
    let client = RpcClient::new_with_commitment(
        "http://localhost:8899".to_string(),
        CommitmentConfig::processed(),
    ...
    assert_eq!(
        option_mint_acct.supply,
        initial_option_token_mint_act.supply - size
    );
    ...
    assert_eq!(
        option_writer_option_token_acct.amount,
        initial_option_writer_option_token_acct.amount - size
    );
    ...
    assert_eq!(
        option_writer_underlying_asset_acct.amount,
        initial_option_writer_underlying_asset_acct.amount
            + (underlying_amount_per_contract * size)
    );
