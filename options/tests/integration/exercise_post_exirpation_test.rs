use crate::{
  option_helpers::{ init_option_market, create_and_add_option_writer},
  solana_helpers,
...
pub fn test_sucessful_exercise_post_expiration() {
  ...
  let expiry = (now + Duration::from_secs(10)).duration_since(UNIX_EPOCH).unwrap().as_secs() as i64;
  ...
  // TODO pick one of the option writers from the OptionMarket account
