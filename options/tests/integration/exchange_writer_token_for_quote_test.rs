...
#[test]
#[serial]
pub fn test_successful_exchange_multiple_writer_token_for_quote_test() {
  let client = RpcClient::new_with_commitment(
    "http://localhost:8899".to_string(),
    CommitmentConfig::processed(),
  );
  ...
  assert_eq!(
    quote_asset_dest_acct.amount,
    initial_quote_asset_dest_acct.amount + (quote_amount_per_contract * size)
  );
  ...
  assert_eq!(
    quote_asset_pool_acct.amount,
    initial_quote_asset_pool_acct.amount - (quote_amount_per_contract * size)
  );
}
