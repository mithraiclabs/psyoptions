...
describe("initializeMarket", () => {
  ...
  describe("underlying and quote assets are the same", () => {
    beforeEach(async () => {
      ...
      optionMarket.quoteAssetMint = optionMarket.underlyingAssetMint;
    });
    it("Should error", async () => {
      try {
        await initOptionMarket(
  ...
  describe("QuoteAssetPool not owned by OptionMarket", () => {
    beforeEach(async () => {
      ...
      // Create a new token account and set it as the quoteAssetPool
      const { tokenAccount } = await initNewTokenAccount(
        provider.connection,
        payer.publicKey,
        underlyingToken.publicKey,
      ...
      optionMarket.quoteAssetPool = tokenAccount.publicKey;
    });
    it("should error", async () => {
      try {
        await initOptionMarket(
